...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'greetings':
				dialogue = CoolUtil.coolTextFile(Paths.txt('greetings/dialog'));
			case 'viber':
				dialogue = CoolUtil.coolTextFile(Paths.txt('viber/dialog'));
		}
			...
			case 'stage':
				{
						...
						var bgLimo:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('DaOtherCrap'));
						bgLimo.scrollFactor.set(0.4, 0.4);
						add(bgLimo);	
					
						var bgStreet:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('ground'));
						bgStreet.scrollFactor.set(1, 1);
			...
			case 'hotel':
				{
						defaultCamZoom = 0.7;
						curStage = 'hotel';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('hotel-wall'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.4, 0.4);
						bg.active = false;
						add(bg);
	
						var bgLimo:FlxSprite = new FlxSprite(-600, -600).loadGraphic(Paths.image('doors'));
						bgLimo.scrollFactor.set(0.45, 0.45);
						add(bgLimo);	
					
						var bgStreet:FlxSprite = new FlxSprite(-600, -250).loadGraphic(Paths.image('floor'));
						bgStreet.scrollFactor.set(1, 1);
						bgStreet.antialiasing = true;
						bgStreet.active = false;
						add(bgStreet);
			...
			case 'hellscape':
				{
						defaultCamZoom = 0.3;
						curStage = 'hellscape';
						var bg:FlxSprite = new FlxSprite(-1000, -600).loadGraphic(Paths.image('magma/sky'));
						bg.antialiasing = true;
						...
						var bgLimo:FlxSprite = new FlxSprite(-1000, -800).loadGraphic(Paths.image('magma/rocks'));
						bgLimo.scrollFactor.set(0.5, 0.5);
						bgLimo.setGraphicSize(Std.int(bgLimo.width * 2));
						add(bgLimo);
					

						var bgIslands:FlxSprite = new FlxSprite(-1500, -1000).loadGraphic(Paths.image('magma/floaty_islands'));
						bgIslands.antialiasing = true;
						bgIslands.scrollFactor.set(0.8, 0.8);
						bgIslands.active = false;
						bgIslands.setGraphicSize(Std.int(bgIslands.width * 2));
						bgIslands.updateHitbox();
						add(bgIslands);

						var bgEscalator:FlxSprite = new FlxSprite(-1500, -800).loadGraphic(Paths.image('magma/hotel_remains'));
						bgEscalator.antialiasing = true;
						bgEscalator.scrollFactor.set(0.8, 0.8);
						bgEscalator.active = false;
						bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 2));
						bgEscalator.updateHitbox();
						add(bgEscalator);

						var bgStreet:FlxSprite = new FlxSprite(-1000, -600).loadGraphic(Paths.image('magma/ground'));
						bgStreet.scrollFactor.set(1, 1);
						bgStreet.antialiasing = true;
						bgStreet.setGraphicSize(Std.int(bgStreet.width * 2));
						bgStreet.active = false;
						add(bgStreet);
				}
			default:
